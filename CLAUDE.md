...
**Skin switching:** For standalone skins (each skin is self-contained), just call `skeleton.setSkinByName(name)` — no `setSkin(null)` beforehand and no `setToSetupPose()` afterward.
